...
class TestMigrosLiveViaWorker(unittest.IsolatedAsyncioTestCase):
    async def test_migros_live_worker_run_saves_products_and_images(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            try:
                db = ProductDatabase(db_path=Path(td) / "products.db")
                queue = JobQueue(db)

                job_id = queue.enqueue("migros", source="test")
                ...
                products = db.get_latest_scrape("migros")
                self.assertGreater(len(products), 0, "expected at least one product from live Migros brand page")

                enriched = db.get_unlinked_products(include_images=True, source="migros")
                ...
                self.assertTrue(sample.get("url") and sample["url"].startswith("https://www.migros.ch/en/product/"))
