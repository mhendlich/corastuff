...
class TestBike24LiveViaWorker(unittest.IsolatedAsyncioTestCase):
    async def test_bike24_live_worker_run_saves_products_and_images(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            try:
                db = ProductDatabase(db_path=Path(td) / "products.db")
                queue = JobQueue(db)

                job_id = queue.enqueue("bike24", source="test")
                ...
                products = db.get_latest_scrape("bike24")
                self.assertGreater(len(products), 0, "expected at least one product from live Bike24 page")

                enriched = db.get_unlinked_products(include_images=True, source="bike24")
                ...
                self.assertTrue(sample.get("url") and sample["url"].startswith("https://www.bike24.com/p"))
