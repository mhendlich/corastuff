...
class TestSportConradLiveViaWorker(unittest.IsolatedAsyncioTestCase):
    async def test_sport_conrad_live_worker_run_saves_products_and_images(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            try:
                db = ProductDatabase(db_path=Path(td) / "products.db")
                queue = JobQueue(db)

                job_id = queue.enqueue("sport_conrad", source="test")
                ...
                products = db.get_latest_scrape("sport_conrad")
                self.assertGreater(len(products), 0, "expected at least one product from live Sport Conrad page")

                enriched = db.get_unlinked_products(include_images=True, source="sport_conrad")
                ...
                self.assertTrue(sample.get("url") and sample["url"].startswith("https://www.sport-conrad.com/"))
