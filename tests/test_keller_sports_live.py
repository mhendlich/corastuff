...
class TestKellerSportsLiveViaWorker(unittest.IsolatedAsyncioTestCase):
    async def test_keller_sports_live_worker_run_saves_products_and_images(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            try:
                db = ProductDatabase(db_path=Path(td) / "products.db")
                queue = JobQueue(db)

                job_id = queue.enqueue("keller_sports", source="test")
                ...
                products = db.get_latest_scrape("keller_sports")
                self.assertGreater(len(products), 0, "expected at least one product from live Keller Sports page")

                enriched = db.get_unlinked_products(include_images=True, source="keller_sports")
                ...
                self.assertTrue(sample.get("url") and sample["url"].startswith("https://keller-sports.de/"))
