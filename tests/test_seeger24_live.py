...
class TestSeeger24LiveViaWorker(unittest.IsolatedAsyncioTestCase):
    async def test_seeger24_live_worker_run_saves_products_and_images(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            try:
                db = ProductDatabase(db_path=Path(td) / "products.db")
                queue = JobQueue(db)

                job_id = queue.enqueue("seeger24", source="test")
                ...
                products = db.get_latest_scrape("seeger24")
                self.assertGreater(len(products), 0, "expected at least one product from live Seeger24 page")

                enriched = db.get_unlinked_products(include_images=True, source="seeger24")
                ...
                self.assertTrue(sample.get("url") and sample["url"].startswith("https://www.seeger24.de/"))
