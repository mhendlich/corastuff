...
class TestDeinVitalShopLiveViaWorker(unittest.IsolatedAsyncioTestCase):
    async def test_dein_vital_shop_live_worker_run_saves_products_and_images(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            try:
                db = ProductDatabase(db_path=Path(td) / "products.db")
                queue = JobQueue(db)

                job_id = queue.enqueue("dein_vital_shop", source="test")
                ...
                products = db.get_latest_scrape("dein_vital_shop")
                self.assertGreater(len(products), 0, "expected at least one product from live Dein-Vital page")

                enriched = db.get_unlinked_products(include_images=True, source="dein_vital_shop")
                ...
                self.assertTrue(sample.get("url") and sample["url"].startswith("https://dein-vital.shop/"))
