...
class TestTransaLiveViaWorker(unittest.IsolatedAsyncioTestCase):
    async def test_transa_live_worker_run_saves_products_and_images(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            try:
                db = ProductDatabase(db_path=Path(td) / "products.db")
                queue = JobQueue(db)

                job_id = queue.enqueue("transa", source="test")
                ...
                products = db.get_latest_scrape("transa")
                self.assertGreater(len(products), 0, "expected at least one product from live Transa page")

                enriched = db.get_unlinked_products(include_images=True, source="transa")
                ...
                self.assertTrue(sample.get("url") and sample["url"].startswith("https://www.transa.ch/"))
