...
class TestManorLiveViaWorker(unittest.IsolatedAsyncioTestCase):
    async def test_manor_live_worker_run_saves_products_and_images(self) -> None:
        try:
            with tempfile.TemporaryDirectory() as td:
                db = ProductDatabase(db_path=Path(td) / "products.db")
                queue = JobQueue(db)

                job_id = queue.enqueue("manor", source="test")
                ...
                products = db.get_latest_scrape("manor")
                self.assertGreater(len(products), 0, "expected at least one product from live manor.ch page")

                enriched = db.get_unlinked_products(include_images=True, source="manor")
                ...
                self.assertTrue(sample.get("url") and sample["url"].startswith("https://www.manor.ch/"))
