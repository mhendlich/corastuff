...
class TestOezpinarLiveViaWorker(unittest.IsolatedAsyncioTestCase):
    async def test_oezpinar_live_worker_run_saves_products_and_images(self) -> None:
        try:
            with tempfile.TemporaryDirectory() as td:
                db = ProductDatabase(db_path=Path(td) / "products.db")
                queue = JobQueue(db)

                job_id = queue.enqueue("oezpinar", source="test")
                ...
                products = db.get_latest_scrape("oezpinar")
                self.assertGreater(len(products), 0, "expected at least one product from live oezpinar.de")

                enriched = db.get_unlinked_products(include_images=True, source="oezpinar")
                ...
                self.assertTrue(sample.get("url") and sample["url"].startswith("https://www.oezpinar.de/"))
