...
class TestMedidorLiveViaWorker(unittest.IsolatedAsyncioTestCase):
    async def test_medidor_live_worker_run_saves_products_and_images(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            try:
                db = ProductDatabase(db_path=Path(td) / "products.db")
                queue = JobQueue(db)

                job_id = queue.enqueue("medidor", source="test")
                ...
                products = db.get_latest_scrape("medidor")
                self.assertGreater(len(products), 0, "expected at least one MEDiDOR product for Blackroll")

                enriched = db.get_unlinked_products(include_images=True, source="medidor")
                ...
                self.assertTrue(sample.get("url") and sample["url"].startswith("https://medidor.ch/"))
