...
class TestKueblerSportLiveViaWorker(unittest.IsolatedAsyncioTestCase):
    async def test_kuebler_sport_live_worker_run_saves_products_and_images(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            try:
                db = ProductDatabase(db_path=Path(td) / "products.db")
                queue = JobQueue(db)

                job_id = queue.enqueue("kuebler_sport", source="test")
                ...
                products = db.get_latest_scrape("kuebler_sport")
                self.assertGreater(len(products), 0, "expected at least one product from live Kübler Sport page")

                enriched = db.get_unlinked_products(include_images=True, source="kuebler_sport")
                ...
                self.assertTrue(sample.get("url") and sample["url"].startswith("https://www.kuebler-sport.de/"))
