...
class TestDecathlonCHLiveViaWorker(unittest.IsolatedAsyncioTestCase):
    async def test_decathlon_ch_live_worker_run_saves_products_and_images(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            try:
                db = ProductDatabase(db_path=Path(td) / "products.db")
                queue = JobQueue(db)

                job_id = queue.enqueue("decathlon_ch", source="test")
                ...
                products = db.get_latest_scrape("decathlon_ch")
                self.assertGreater(len(products), 0, "expected at least one Decathlon.ch product for Blackroll")

                enriched = db.get_unlinked_products(include_images=True, source="decathlon_ch")
                ...
                self.assertTrue(sample.get("url") and sample["url"].startswith("https://www.decathlon.ch/de/p/"))
