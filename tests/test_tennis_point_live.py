...
class TestTennisPointLiveViaWorker(unittest.IsolatedAsyncioTestCase):
    async def test_tennis_point_live_worker_run_saves_products_and_images(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            try:
                db = ProductDatabase(db_path=Path(td) / "products.db")
                queue = JobQueue(db)

                job_id = queue.enqueue("tennis_point", source="test")
                ...
                products = db.get_latest_scrape("tennis_point")
                self.assertGreater(len(products), 0, "expected at least one product from Tennis-Point page")

                enriched = db.get_unlinked_products(include_images=True, source="tennis_point")
                ...
                self.assertTrue(sample.get("url") and sample["url"].startswith("https://www.tennis-point.de/"))
