...
class TestDigitecLiveViaWorker(unittest.IsolatedAsyncioTestCase):
    async def test_digitec_live_worker_run_saves_products_and_images(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            try:
                db = ProductDatabase(db_path=Path(td) / "products.db")
                queue = JobQueue(db)

                job_id = queue.enqueue("digitec_ch", source="test")
                ...
                products = db.get_latest_scrape("digitec_ch")
                self.assertGreater(len(products), 0, "expected at least one Digitec product for Blackroll")

                enriched = db.get_unlinked_products(include_images=True, source="digitec_ch")
                ...
                self.assertTrue(sample.get("url") and sample["url"].startswith("https://www.digitec.ch/en/s1/product/"))
