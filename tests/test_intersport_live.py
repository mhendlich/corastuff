...
class TestIntersportLiveViaWorker(unittest.IsolatedAsyncioTestCase):
    async def test_intersport_live_worker_run_saves_products_and_images(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            db = ProductDatabase(db_path=Path(td) / "products.db")
            queue = JobQueue(db)

            job_id = queue.enqueue("intersport", source="test")
            ...
            products = db.get_latest_scrape("intersport")
            self.assertGreater(len(products), 0, "expected at least one product from live Intersport page")

            enriched = db.get_unlinked_products(include_images=True, source="intersport")
            ...
            self.assertTrue(sample.get("url") and sample["url"].startswith("https://www.intersport.de/"))
